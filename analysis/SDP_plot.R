  ...
  for (q_out in qz)
  {
    for (qbio in bios)
    {
      for (scen in c(1,3,5,7,9,11))
      {
        ...
        d4prime<-read.csv(paste("~/Desktop/OneDrive - McGill University/GitHub/eab_mgmt/output/RoT_vecptime",frac_site,frac_spread,q_in,qbio, ".csv", sep="_"))[,1:6]
...
corners<-par("usr")
text("50%             30%             10%",y=c(8), x=corners[2]+.75, cex=0.5, srt=270)
text("50%             30%             10%",y=c(5), x=corners[2]+.75, cex=0.5, srt=270)
text("50%             30%             10%",y=c(2), x=corners[2]+.75, cex=0.5, srt=270)

mtext(side=2, "Quarantine efficiency",line=1, cex=0.5)
corners = par("usr") #Gets the four corners of plot area (x1, x2, y1, y2)
text(x = corners[2]+1.5, y = mean(corners[3:4]),"Biological Control efficiency",cex=0.5, srt = 270)
...
points(x=c(0.1,0.3,0.5), y=c(1239440,1050990,767630),col="red", pch=23) 
legend('topright', legend=c("30% Quarantine Efficiency","60% Quarantine Efficiency", "90% Quarantine Efficiency", "Biocontrol Only"), pch=c(19,23), col=c(viridis(27)[c(1,13,25)], "red"), cex=0.75)
...
obj<-obj[2:nrow(obj),]
layout(matrix(c(1,1), ncol=1, byrow=TRUE), heights=c(1))
par(mar=c(4,4,2,2))
par(oma=c(0,0,0,0))
plot(y=obj$obj,x=(obj$frac_site), col=alpha(viridis(9)[as.factor(paste0(obj$q_in, obj$qbio))],0.75), xlab="Site-focused budget proportion", ylab="Exposed ash street trees", pch=19, xlim=c(-0.05,1.05), ylim=c(100000,1300000))

library(readxl)
dat<-read.csv('~/Downloads/postdocdat.csv')
abline(h=dat$Exposed.Ash.Street.Trees..thousands.*1000, lty=2,col=viridis(9))
par(mai=c(0.4,0.5,0.1,0.5))
image(1:9,1,matrix(1:9), col=viridis(9), axes=FALSE, ann=F)
axis(1,labels=c("30% Q, 10% C","90% Q, 50% C"),at=c(seq(1,9,length.out=2)), cex.axis=0.75)
mtext(side=1, "Scenario efficiency",line=1, cex=0.75)
