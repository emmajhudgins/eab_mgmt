  ...
  q_out=q_in
    for (qbio in bios)
    {
      for (scen in c(1,3,5,7,9,11))
      {
       ...
       # d4prime<-read.csv('vecptime_0.9_0.1.csv', header=F)[,2:8]/1000
...
dat<-read.csv('postdocdat_street_debug.csv')
...
for (q_in in qz)
{
  ...
  for (qbio in bios)
  {
    d4prime<-read.csv(paste("vecptime",q_in,qbio, "doublecost.csv", sep="_"), header=F)[,2:8]/1000
...
plot(y=obj$obj,x=(obj$q_in), col=alpha(viridis(9)[as.factor(paste0(obj$q_in, obj$qbio))],0.75),xlab="Quarantine efficiency", ylab="Exposed ash street trees", pch=19, xlim=c(0,0.95), ylim=c(0,900000), main="Fast Dispersal Scenario", cex.main=0.75, axes=F)
axis(1,labels=c("","30%", "60%", '90%'), at=c(0,0.3,0.6,0.9), outer=F)
axis(2, labels=c('0',"200000", "400000", '600000', '800000', '1000000'), at=c(0,200000,400000,600000,800000, 1000000), outer=F, las=2, cex.axis=0.75, hadj=0.8)
points(y=dat$Exposed.Ash.Street.Trees..thousands.*1000, x=dat$Quarantine.efficiency+0.05, lty=2,col=viridis(9), pch=5)
par(mai=c(0.4,0.5,0.1,0.6))
...
for (q_in in qz)
{
  ...
  for (qbio in bios)
  {
    d4prime<-read.csv(paste("vecptime",q_in,qbio, "mult.csv", sep="_"), header=F)[,2:8]/1000
    obj<-rbind(obj, setNames(c(q_in,q_out,qbio,sum(sweep(as.matrix(d4prime),MARGIN=1,as.vector(V_i[prez[,1]]+1),"*"))),names(obj)))
  }
}
obj<-obj[2:nrow(obj),]
...
plot(y=obj$obj,x=(obj$q_in), col=alpha(viridis(9)[as.factor(paste0(obj$q_in, obj$qbio))],0.75),xlab="Quarantine efficiency", ylab="Exposed ash street trees", pch=19, xlim=c(0,0.95), ylim=c(0,900000), main="Multiple Actions Scenario", cex.main=0.75, axes=F)
