...
for (q_out in qz)
{
  for (qbio in bios)
  {
    ...
    for (scen in c(1,3,5,7,9,11))
    {
      ...
      bc_pp_out<-bc_pp_in<-pp_bio<-matrix(0,1799,total_time+8)
      ...
      for (time in 1:(total_time+8))
      {
        ...
        if (time>2)
        {
          ...
          if (time>(floor(total_time))){
            ...
            for (i in 1:1799)
            {
              bc_pp_out[i,time]<-sum((V_i[prez[which(shortpath_out==i),1]]+1)*(1-vecP[which(shortpath_out==i)])*vecP[i])
              bc_pp_in[i,time]<-sum((vecP[which(shortpath_in==i)])*(V_i[prez[i,1]]+1)*(1-vecP[i]))
