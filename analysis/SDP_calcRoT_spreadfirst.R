             ...
             # write.csv(qq2, file=paste("transmatM_", spp,time, ".csv", sep=""), row.names=F)
              ...
              while(is.infinite(tt)==F&cost2<=B*frac_site& sum(ce_site[which(ce_site>=tt)], na.rm=T)!=0 & tt<3599)
              {
                ...
                if (length(tt)>0){
                 ...
                 {tt=min(ce_site[which(ce_site>tt)],na.rm=T)+1
                  if (tt>3598)
                  {
                    break
                  }}
  ...
  #            vecP[which(prez[,spp]==Psource)]=1
            ...
            write.csv(M_big, file=paste("management_test",frac_site,frac_spread,q_in,q_out,qbio,".csv", sep="_"), row.names=F)
              }
               }}
...
for (time in 6:11)
{
 ...
 erad<-unlist(apply(mgmt_itme[((4*1799+1):(5*1799)),],2, function(x){length(which(x==1))}))
cost_each<-matrix(c(unlist(lapply(qin,function(x){(x)*(646863/309)})), unlist(lapply(qout,function(x){(x)*(646863/309)})),unlist(lapply(bio,function(x){(x)*(141519)})),c(d_time[mgmt[[6]][length(mgmt[[6]])]-3*1799,1]*31000*2500,0,0,0,d_time[mgmt[[10]][length(mgmt[[10]])]-3*1799,5]*31000*2500)),nrow=4,ncol=5,byrow=T)
