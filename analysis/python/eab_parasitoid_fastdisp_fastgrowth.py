...
randoms =numpy.array([random.uniform(0,0.0001) for _ in sites])
...
for rr in range(0,3): #iterate over efficiency scenarios
    for qq in range(0,3): 
           ...
           #Growth
            ...
            m.addConstrs((d4prime[loc, year] >= c_4[loc,year]*phi*r-(3000/phi) *((M[L+loc, year]+M[2*L+loc,year]+c_8[loc,year-1])) for loc in sites for year in range(2,6)) , name="growth")#if no management taken, populations can't go extinct (maintained at phi*r)
            ...
            m.addConstrs(((c_8[ii,year-1]<=quicksum(M[3*L+ii,year-1]+M[3*L+adj_list[ii-1][0][jj],year-2] for jj in range(len(adj_list[ii-1][0])))) for ii in not_adj_bio_2020 for year in range(3,5)), name="orstatement")
            #m.addConstrs(((dprime[ii,year] >= d[ii,year]-((1000/phi) *c_8[ii,year-2])) for ii in sites for year in range(3,6)), name = "nobiocontrol)")
            m.addConstrs(((dprime[ii,year] >= (1-eff_vec[3*L+ii-1])*d[ii,year]-((1000/phi) *(1-c_8[ii,year-1]))) for ii in sites for year in range(2,6)), name = "biocontrol")
            ...
            m.addConstrs(((c_8[ii,year-1]>=M[3*L+jj,year-2]) for jj in adj_list[ii-1][0].astype(int) for year in range(3,6)), name = "parasitoidDisp")

           # m.addConstrs(((c_8[ii,1]==1) for ii in bio_2020 for year in range(1,2)), name = "biohist2")
            m.addConstrs(((c_8[ii,2]==1) for ii in pandas.Series([adj_list[q] for q in bio_2020][0])[0].astype(int) for year in range(1,2)), name = "biohist2")
            m.addConstrs(((c_8[ii,1]==1) for ii in pandas.Series([adj_list[q] for q in bio_2015][0])[0].astype(int) for year in range(1,2)), name = "biohist2")
           # m.addConstrs(((c_8[ii,1]==1) for ii in pandas.Series([adj_list[q] for q in bio_2010][0])[0].astype(int) for year in range(1,2)), name = "biohist2")
            ...
            m.addConstrs(((d4prime[loc, year] >= c_4[loc,year]*phi*r-(phi*r)*(c_8[loc,year-1]+(1-M[loc, year]))) for loc in sites for year in range(3,6)), name="growth3")#if no management taken, populations can't go extinct (maintained at phi*r)
    ...
    ##          M[sites3, year].start=mgmt.iloc[sites3-1,year-1]    
            m.setParam('LogFile', 'eab_parasitoid_fastdisp_fastgrowth_{0}_{1}.log'.format(rr,qq)) #unique logfile
            ...
            M3 = pandas.Series(M2)
            M4 = M3.unstack()
            M4.to_csv('../../output/M_{0}_{1}_fastdisp_fastgrowth.csv'.format(eff_quar_out,eff_bio), index=False,header=False)
