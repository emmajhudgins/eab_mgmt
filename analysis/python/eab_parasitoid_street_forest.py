...
ForestVol = pandas.io.parsers.read_csv('../../data/ash_forestvol.csv') #modelled street tree distributions (inc. for ash trees) from HUdgins et al. 2021
...
for rr in range(0,3): #iterate over efficiency scenarios
    for qq in range(0,3): 
            for weight in area_bud:
                ...
                m.setObjective(quicksum(weight*(d[loc, year]*(ForestVol.iloc[prez.iloc[loc-1]-1]+0.000001))+(1-weight)*(d[loc, year]*(HostVol.iloc[prez.iloc[loc-1]-1,19]+0.000001) ) for loc in sites for year in range(1+1,len(time)+2) ), GRB.MINIMIZE)
               ...
               #Growth
                ...
                m.addConstrs(((c_8[ii,3]==1) for ii in pandas.Series([adj_list[q] for q in bio_2020][0])[0].astype(int) for year in range(1,2)), name = "biohist2")
                m.addConstrs(((c_8[ii,2]==1) for ii in pandas.Series([adj_list[q] for q in bio_2015][0])[0].astype(int) for year in range(1,2)), name = "biohist2")
        ...
        ##          M[sites3, year].start=mgmt.iloc[sites3-1,year-1]    
                m.setParam('LogFile', 'eab_parasitoid_{0}_{1}_{2}_streetforest.log'.format(weight,rr,qq)) #unique logfile
                ...
                M3 = pandas.Series(M2)
                M4 = M3.unstack()
                M4.to_csv('../../output/M_{0}_{1}_{2}_streetforest.csv'.format(weight,eff_quar_out,eff_bio), index=False,header=False)
             #   m.write('../../output/model_{0}_{1}_{2}_area.mps'.format(bud,eff_quar_out,eff_bio)) #create .mps file for gurobi_cl
              #  m.write('../../output/modelstart_{0}_{1}_{2}_area.sol'.format(bud,eff_quar_out,eff_bio)) # use as initial solution in gurobi_cl