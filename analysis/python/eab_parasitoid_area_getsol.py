...
randoms =numpy.array([random.uniform(1.001,1.01) for _ in sites])
...
for rr in range(0,3): #iterate over efficiency scenarios
    for qq in range(0,3): 
            ...
            m.setObjective(quicksum(c_4[loc, year]*randoms[loc-1] for loc in sites for year in range(1+1,len(time)+1) ), GRB.MINIMIZE)
           ...
           #Growth
            ...
            mgmt=pandas.io.parsers.read_csv("../../output/M_{0}_{1}_area.csv".format(eff_quar_in, eff_bio), header=None)#starting condition
    ...
    #       implement starting condition
            ...
            m.setParam('LogFile', 'eab_parasitoid_{0}_{1}_area.log'.format(rr,qq)) #unique logfile
            ...
            M3 = pandas.Series(M2)
            M4 = M3.unstack()
            M4.to_csv('../../output/vecptime_{0}_{1}_area.csv'.format(eff_quar_out,eff_bio), index=False,header=False)
