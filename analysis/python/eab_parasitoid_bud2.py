...
for rr in range(1,2): #iterate over efficiency scenarios
    for qq in range(2,3): 
        for bud in quar_bud:
            ...
            start= list(bio_2020)
            start.extend(x for x in [adj_list[q][0] for q in bio_2015][0] if x not in bio_2020)
            nostart= list(x for x in sites if x not in start)
            second = list()
            second.extend(x for x in [adj_list[q][0] for q in bio_2020][0] if x not in bio_2015)
...
#            m.addConstrs(((c_8[ii,year]<=quicksum(M[3*L+ii,year]+M[3*L+adj_list[ii-1][0][jj],year-2] for jj in range(len(adj_list[ii-1][0])))) for ii in not_adj_bio_2020 for year in range(4,5)), name="orstatement")
            ...
            m.addConstrs(((c_8[ii,1]==1) for ii in pandas.Series([adj_list[q] for q in bio_2015][0])[0].astype(int)), name = "biohist3")
            m.addConstrs(((c_8[ii,2]==1) for ii in pandas.Series([adj_list[q] for q in bio_2020][0])[0].astype(int)), name = "biohist4")
            m.addConstrs(((c_8[ii,1]==0) for ii in nostart), name = "biohist5")
            ...
            mgmt=pandas.io.parsers.read_csv("../../output/management_test_{0}_{1}_{2}_{3}_.csv".format(numpy.around(1-bud,1), bud,eff_quar_in, eff_bio))#starting condition
        ...
        #       implement starting condition
            ...
            m.setParam('LogFile', 'eab_parasitoid_{0}_{1}_but.log'.format(rr,qq)) #unique logfile
            ...
            M3 = pandas.Series(M2)
            M4 = M3.unstack()
            M4.to_csv('../../output/M_{0}_{1}_{2}_bud.csv'.format(eff_quar_out,eff_bio,bud), index=False,header=None)
