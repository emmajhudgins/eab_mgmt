...
for rr in range(0,3): #iterate over efficiency scenarios
    for qq in range(0,3): 
        ...
        m.addConstrs(((M[3*L+ii,year]<=c_4[loc,year]) for loc in sites for year in time), name = "biocontrol3") # can only do biocontrol at places initially invaded
       ...
       #Growth
        ...
        m.addConstrs(((c_8[ii,year-2]<=quicksum(M[3*L+ii,year-2]+M[3*L+adj_list[ii-1][0][jj],year-4] for jj in range(len(adj_list[ii-1][0])))) for ii in sites for year in range(5,6)), name="orstatement")
        ...
        m.addConstrs(((c_8[ii,year-2]>=M[3*L+ii,year-2]) for ii in sites for year in range(3,6)), name = "biocontrol2")
        m.addConstrs(((c_8[ii,year-2]>=M[3*L+jj,year-4]) for jj in adj_list[ii-1][0].astype(int) for year in range(5,6)), name = "parasitoidDisp")
