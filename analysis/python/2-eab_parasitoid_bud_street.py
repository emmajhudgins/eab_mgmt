...
source = 2951 # source cell s
time = range(1,7+1) #35 years (to account for lagged impact of biocontrol)
time_sub = range(1,5+1) #25 years (actual management horizon for decision variables)
...
#iterate over efficiency scenarios

for rr in range(0,3):
    for qq in range(0,3): 
        for bud in quar_bud:
            effs_quar=[0.3,0.6,0.9] #efficiency scenarios for quarantine
            ...
            all_comb = range(1, L * n_actions+1) # vector containing all decision variables, L*n_actions
            ...
            dprime = m.addVars(sites,time, vtype=GRB.CONTINUOUS, name="dprime", lb=0,ub=1000) # w',density variables before and after different model equations/constraints
            d2prime = m.addVars(sites,time, vtype=GRB.CONTINUOUS, name="d2prime", lb=0,ub=1000)#w'',
            d3prime = m.addVars(sites,time, vtype=GRB.CONTINUOUS, name="d3prime", lb=0,ub=3000/phi)#w''', #3000/phi is a reasonable upper bound to limit size of model matrix
            d4prime = m.addVars(sites,time, vtype=GRB.CONTINUOUS, name="d4prime",lb=0,ub=3000/phi) #w''''
            d_out=m.addVars(sites, time, vtype=GRB.CONTINUOUS, name="d_out", lb=0,ub=1000) #w_out
    
            #different indicator constraints
            c_4 = m.addVars(sites,time, vtype=GRB.BINARY, name="c_4") #I^phi
            c_5 = m.addVars(sites,time, vtype=GRB.BINARY, name="c_5") #I^phi2
            c_6 = m.addVars(sites,time, vtype=GRB.BINARY, name="c_6")#I^<1
            c_7 = m.addVars(sites,time, vtype=GRB.BINARY, name="c_7")#I>1
            ...
            vecptime = pandas.io.parsers.read_csv("../../output/RoT_vecptime_1_0_{0}_{1}_.csv".format(eff_quar_in, eff_bio))*1000 #density ix x1000 to reduce orders of magnitude in model matrix
            ...
            mgmt=pandas.io.parsers.read_csv("../../output/management_test_{0}_{1}_{2}_{3}_.csv".format(round(1-bud,1),bud,eff_quar_in,eff_bio))#starting condition
            ...
            m.setParam('LogFile', 'eab_parasitoid_{0}_{1}_{2}_bud.log'.format(bud,rr,qq)) #unique logfile
            ...
            M3 = pandas.Series(M2)
            M4 = M3.unstack()
            M4.to_csv('../../output/M_{0}_{1}_{2}_bud.csv'.format(bud,eff_quar_out,eff_bio), index=False,header=None)
           ...
           # m.write('../../output/modelstart_{0}_{1}.sol'.format(eff_quar_out,eff_bio)) # use as initial solution in gurobi_cl
            ...
            M3 = pandas.Series(M2)
            M4 = M3.unstack()
            M4.to_csv('../../output/vecptime_{0}_{1}_{2}_bud.csv'.format(bud,eff_quar_out,eff_bio), index=False,header=None)
