...
for rr in range(0,3): #iterate over efficiency scenarios
    ...
    ##          M[sites3, year].start=mgmt.iloc[sites3-1,year-1]    
            m.setParam('LogFile', 'eab_parasitoid_nothresh_{0}_{1}.log'.format(rr,qq)) #unique logfile
            ...
            M3 = pandas.Series(M2)
            M4 = M3.unstack()
            M4.to_csv('../../output/M_nothresh_{0}_{1}.csv'.format(eff_quar_out,eff_bio), index=False,header=False)
