...
HostVol = pandas.io.parsers.read_csv('../../data/ash_forestvol.csv') #modelled street tree distributions (inc. for ash trees) from HUdgins et al. 2021
...
for rr in range(0,3): #iterate over efficiency scenarios
    for qq in range(0,3): 
            ...
            m.setObjective(quicksum(d[loc, year]*(HostVol.iloc[prez.iloc[loc-1]-1]+0.000001) for loc in sites for year in range(1+1,len(time)+2)), GRB.MINIMIZE)
    ...
    ##          M[sites3, year].start=mgmt.iloc[sites3-1,year-1]    
            m.setParam('LogFile', 'eab_parasitoid_forest_{0}_{1}.log'.format(rr,qq)) #unique logfile
            ...
            M3 = pandas.Series(M2)
            M4 = M3.unstack()
            M4.to_csv('../../output/M_forest_{0}_{1}.csv'.format(eff_quar_out,eff_bio), index=False,header=False)
