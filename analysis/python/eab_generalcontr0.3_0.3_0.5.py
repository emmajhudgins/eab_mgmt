...
for ii in sites:
    ...
    m.addConstrs(((quicksum(M[loc,year]for loc in j) == 1) for year in time), name = 'OneAction_{0}'.format(ii)) 

m.addConstrs(((c_4[loc,year]==1)>>(M[4*L+loc,year]==0) for loc in sites2 for year in time), name="erad2bud")
m.addConstrs(((M[4*L+loc,year]==1)>>(d[loc,year]<=erad_thresh) for loc in sites2 for year in time), name="eradbud2")
...
m.addConstrs(((M[4*L+loc,year]==1)>>((B_erad[loc,year]==d[loc,year]*cost_erad)) for loc in sites for year in time), name="eradbud")
m.addConstrs(((M[4*L+loc,year]==0)>>((B_erad[loc,year]==0)) for loc in sites for year in time), name="noeradbud")
m.addConstrs((B_nonerad[year]+quicksum(B_erad[loc,year]for loc in sites) <=B for year in time), name = "TotalBudget")


#Management
m.addConstrs(((M[loc,year]==1)>>(dprime[loc,year] == d[loc,year]) for loc in sites for year in time), name = "Effectof2Mgmt1" )
m.addConstrs(((M[L+loc,year]==1)>>(dprime[loc,year] == d[loc,year]) for loc in sites for year in time), name = "Effectof2Mgmt2" )
m.addConstrs(((M[2*L+loc,year]==1)>>(dprime[loc,year] == d[loc,year]) for loc in sites for year in time), name = "Effectof2Mgmt3" )
#Lower threshold
m.addConstrs((c_4[loc,year]>=(dprime[loc,year]-phi)/(1000/phi) for loc in sites for year in time), name="LT1")
m.addConstrs(((c_4[loc,year]==0)>>(d2prime[loc,year]==0) for loc in sites for year in time), name="LT2")
m.addConstrs(((c_4[loc,year]==1)>>(d2prime[loc,year]==dprime[loc,year]) for loc in sites for year in time), name="LT3")
#Dispersal 
m.addConstrs(((M[2*L+loc,year]==0)>>(d_out[loc,year] == d2prime[loc,year]) for loc in sites for year in time), name="noquar_2out")
#Growth
m.addConstrs(((c_5[loc,year]==1)>>(d4prime[loc, year] == d3prime[loc, year]*r) for loc in sites2 for year in time), name="growth1")
m.addConstrs(((d4prime[loc, year] >= d3prime[loc, year]) for loc in sites2 for year in time), name="growth2")
m.addConstrs(((M[loc, year]==1)>>(d4prime[loc, year] >= c_4[loc,year]*phi*r) for loc in sites2 for year in time), name="growth3")
m.addConstrs(((c_5[loc,year]>=(d3prime[loc,year]-phi)/(3000/phi)) for loc in sites2 for year in time), name="growth4")
## Cap d4 prime after growth by 1 - convert to starting density at next timestep
m.addConstrs((((c_6[loc,year]+c_7[loc,year] == 1)) for loc in sites2 for year in time), name="max_d1")
m.addConstrs(((c_6[loc,year]==1)>>(d[loc, year+1] == d4prime[loc, year]) for loc in sites2 for year in time), name="max_d2")
m.addConstrs(((c_7[loc,year]==1)>>(d[loc, year+1] == 1000) for loc in sites2 for year in time), name="max_d3")
...
m.addConstrs(((d4prime[source,year]==1000*r) for year in range(1, len(time)+1)), name="source_den2")
...
m.setParam('PumpPasses', 1000)
m.setParam('MinRelNodes', 1000)
...
m.addConstrs(((M[2*L+ii,year]==1)>>(d_out[ii,year] == (1-eff_vec[2*L+ii-1])*d2prime[ii,year]) for ii in sites for year in time), name = "quar_out2" )
m.addConstrs(((M[3*L+ii,year]==1)>>(dprime[ii,year] == (1-eff_vec[3*L+ii-1])*d[ii,year]) for ii in sites for year in time), name = "EffectofMgmt2" )
m.addConstrs(((M[4*L+ii,year]==1)>>(dprime[ii,year] == (1-eff_vec[4*L+ii-1])*d[ii,year]) for ii in sites for year in time), name = "EffectofMgmt3" )
...
m.addConstrs(((M[L+ii,year]==1)>>(d3prime[ii,year] == tij[year-1,ii-1,ii-1]*d2prime[ii,year]+(1-eff_vec[L+ii-1])*(quicksum(tij[year-1,j-1,ii-1]*d_out[j,year] for j in numpy.ndarray.tolist(full_out[year-1][ii-1][0]))-tij[year-1,ii-1,ii-1]*d_out[ii,year])) for ii in sites for year in time), name = "quar_in")
m.addConstrs(((M[L+ii,year]==0)>>(d3prime[ii,year] == tij[year-1,ii-1,ii-1]*d2prime[ii,year]+quicksum(tij[year-1,j-1,ii-1]*d_out[j,year] for j in numpy.ndarray.tolist(full_out[year-1][ii-1][0]))-tij[year-1,ii-1,ii-1]*d_out[ii,year]) for ii in sites for year in time), name="noquar_in")
