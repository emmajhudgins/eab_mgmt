...
for rr in range(0,1): #iterate over efficiency scenarios
    for qq in range(0,1): 
       ...
       # m.addConstrs(((dprime[ii,year] >= d[ii,year]-((1000/phi) *c_8[ii,year-1])) for ii in sites for year in range(3,8)), name = "nobiocontrol)")
        ...
        m.addConstrs(((c_8[ii,year]>=M[3*L+jj,year-1]) for ii in sites for jj in adj_list[ii-1][0].astype(int) for year in range(2,5)), name = "parasitoidDisp")
...
#            no_3= list(x for x in sites if x not in third)
        ...
        m.addConstrs(((c_8[ii,year]<=M[3*L+ii,year]+quicksum(M[3*L+jj,year-1] for ii in sites for jj in adj_list[ii-1][0].astype(int))) for ii in sites for year in range(2,6)), name = "biocontrol2")
