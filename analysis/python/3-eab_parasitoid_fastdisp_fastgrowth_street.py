...
for rr in range(0,1): #iterate over efficiency scenarios
    for qq in range(2,3): 
        #efficiencies 
        ...
        vecptime = pandas.io.parsers.read_csv("../../output/RoT_vecptime_0_1_{0}_{1}_.csv".format(eff_quar_in, eff_bio))*1000 #density ix x1000 to reduce orders of magnitude in model matrix
        ...
        m.addConstrs(((dprime[ii,year] >= (1-eff_vec[3*L+ii-1])*d[ii,year]-((1000/phi) *(1-c_8[ii,year-1]))) for ii in sites for year in range(2,8)), name = "biocontrol")
       # m.addConstrs(((dprime[ii,year] >= d[ii,year]-((1000/phi) *c_8[ii,year-1])) for ii in sites for year in range(3,8)), name = "nobiocontrol)")
        ...
        m.addConstrs(((c_8[ii,year]>=M[3*L+ii,year]) for ii in sites for year in range(1,5)), name = "biocontrol2")
        m.addConstrs(((c_8[ii,year]>=M[3*L+jj,year-1]) for jj in adj_list[ii-1][0].astype(int) for year in range(2,5)), name = "parasitoidDisp")

        # take into account historical biocontrol prior to today (because of time lags)
        start= list()
        for j in range(0, len(bio_2015)):
            start.extend(x for x in [adj_list[q][0] for q in bio_2020][j])
...
#            no_3= list(x for x in sites if x not in third)
        ...
        m.addConstrs(((c_8[ii,year]<=M[3*L+ii,year]+quicksum(M[3*L+jj,year-1] for jj in adj_list[ii-1][0].astype(int))) for ii in sites for year in range(2,6)), name = "biocontrol2")
...
#            m.addConstrs(((c_8[ii,year]<=quicksum(M[3*L+ii,year]+M[3*L+adj_list[ii-1][0][jj],year-2] for jj in range(len(adj_list[ii-1][0])))) for ii in not_adj_bio_2020 for year in range(4,5)), name="orstatement")
      ...
      #  m.addConstrs(((d4prime[loc, year] >= (c_4[loc,year]*phi*r)-((3000/phi)*(M[L+loc, year]+M[2*L+loc,year]+c_8[loc,year-1]+c_8[loc,year-2]))) for loc in sites for year in range(3,6)) , name="growth")#if no management taken, populations can't go extinct (maintained at phi*r)
        ...
        mgmt=pandas.io.parsers.read_csv("../../output/M_{0}_{1}_fdfg.csv".format(eff_quar_in, eff_bio), header=None)#starting condition
        ...
        M3 = pandas.Series(M2)
        M4 = M3.unstack()
        M4.to_csv('../../output/M_{0}_{1}_fdfg.csv'.format(eff_quar_out,eff_bio), index=False,header=None)
       ...
       # m.write('../../output/modelstart_{0}_{1}.sol'.format(eff_quar_out,eff_bio)) # use as initial solution in gurobi_cl
        ...
        M3 = pandas.Series(M2)
        M4 = M3.unstack()
        M4.to_csv('../../output/vecptime_{0}_{1}_fdfg.csv'.format(eff_quar_out,eff_bio), index=False,header=None)
