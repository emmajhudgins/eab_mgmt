...
for qbio in effs_bio:
    ...
    m.addConstrs((M[loc,year]==0 for loc in range_quar_in for year in time), name= "no qin")
    m.addConstrs((M[loc,year]==0 for loc in range_quar_out for year in time), name= "no qout")        
   ...
   #m.optimize()
    m.write('model_{0}_noquar.mps'.format(qbio))
