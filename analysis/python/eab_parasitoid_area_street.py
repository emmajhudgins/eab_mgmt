...
for rr in range(0,3): #iterate over efficiency scenarios
    for qq in range(0,3): 
            for weight in area_bud:
                ...
                m.setObjective(quicksum(weight*(c_4[loc, year]*randoms[loc-1])+(1-weight)*(d[loc, year]*(HostVol.iloc[prez.iloc[loc-1]-1,19]+0.000001) ) for loc in sites for year in range(1+1,len(time)+1) ), GRB.MINIMIZE)
        ...
        ##          M[sites3, year].start=mgmt.iloc[sites3-1,year-1]    
                m.setParam('LogFile', 'eab_parasitoid_{0}_{1}_{2}_area_street.log'.format(weight,rr,qq)) #unique logfile
                ...
                M3 = pandas.Series(M2)
                M4 = M3.unstack()
                M4.to_csv('../../output/M_{0}_{1}_{2}_area_street.csv'.format(weight,eff_quar_out,eff_bio), index=False,header=False)
