...
for (q_out in qz)
{
  for (qbio in bios)
  {
    ...
    for (scen in c(1))
    {
      ...
      for (time in 1:(total_time+8))
      {
        ...
        if (time>2)
        {
          ...
          if (time>(floor(total_time))){
            ...
            ce_spread[c(which(bc_pp_in[,time]==0), which(bc_pp_out[,time]==0)+L)]<-NA
            tt=min(ce_spread,na.rm=T)
            cost3=cost4=0
            while(is.infinite(tt)==F& cost4<B*frac_spread & sum(ce_spread[which(ce_spread>=tt)], na.rm=T)!=0)
            {
              if(which(ce_spread==tt)%in%c(mgmt[[time]], mgmt[[time]]-L,mgmt[[time]]+L )==F)
              {
            ...
            ce_site[which(vecP<0.027916)]<-NA
            tt=min(ce_site,na.rm=T)
            cost=cost2=0
            while(is.infinite(tt)==F&cost2<B*frac_site& sum(ce_site[which(ce_site>=tt)], na.rm=T)!=0 & tt<L)
            {
              if (which(ce_site==tt)%in%c(mgmt[[time]], mgmt[[time]]-L,mgmt[[time]]-(2*L),mgmt[[time]]-(3*L),mgmt[[time]]+L,mgmt[[time]]+(2*L),mgmt[[time]]+(3*L))==F)
              {
                mgmt[[time]]<-c(mgmt[[time]],which(ce_site==tt)+2*L)
                cost=cost+cost_site[which(ce_site==tt)]}
              tt=min(ce_site[which(ce_site>tt)],na.rm=T)
              if (length(tt)>0){
                while (is.infinite(tt)==F | (cost_site[which(ce_site==tt)]>(B-cost2) ) )
                {
      ...
      write.csv(vecP_time[,5:12], file=paste("../output/RoT_vecptime",frac_site,frac_spread,q_in,qbio,".csv", sep="_"), row.names=F)

      M_big<-matrix(0,L*5,5)
      M_big[1:L,]<-1
      for (i in 1:5){
        M_big[mgmt[[i+5]]+L,i]<-1
        M_big[which((1:L)%in%c(mgmt[[i+5]],mgmt[[i+5]]-L,mgmt[[i+5]]-(2*L))),i]<-0
      }

       M_big[which(prez[,spp]%in%subset(biohist,V2==2020)$V3)+3*L, 1]<-0   
      ...
      M_big[adj_2015+(3*L), 1]<-0
       ...
       M_big[adj_2020+(3*L), 2]<-0
       for (i in 1:L)
       {
         for (j in 1:5)
         {
         if (sum(M_big[c(L+i, 2*L+i, 3*L+i),j])==0)
         {
           M_big[i,j]<-1
         }
         }
