...
for (q_in in qz)
{
  ...
  for (qbio in bios)
  {
    for (scen in c(1,3,5,7,9,11))
    {
      ...
      d4prime<-read.csv(paste("../output/RoT_vecptime",frac_site,frac_spread,q_in,qbio, ".csv", sep="_"))
...
# 
 mgmt_itme<-read.csv('../output/street/M_0.9_0.5.csv') # examine gurobi Management scenario, change file name to different efficiency scenaris
